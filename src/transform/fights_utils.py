...
#     Groups by ['fight_url', 'fighter'] and for each column:
#       - Takes the maximum of 'round'
...
#     # Determine columns to aggregate (all except the grouping keys)
...
#             agg_dict[col] = 'max'
#         else:
#             # For numeric columns, sum; for non-numeric, take the first.
...
#     """Compute cumulative statistics for each metric column."""
#     cum_dict = {}
...
#     """Compute fixed-window rolling aggregates (incoming) for each metric."""
#     roll_dict = {}
...
# # ---------------------------
# # (C) Opponent Stats: Adding Opponent Data and Computing Aggregates
# # ---------------------------
# def add_opponent_stats(df: pd.DataFrame, metric_cols: list) -> pd.DataFrame:
#     """
#     For each fight, add the opponent's aggregated metrics.
    
#     Creates a self-merge of the fight-level DataFrame so that for each row (fighter in a fight),
#     the opponent's metrics are available with a prefix 'opp_'.
#     """
#     # Create a helper DataFrame for opponent stats.
#     opp_df = df[['fight_url', 'fighter'] + metric_cols].copy()
#     opp_df = opp_df.rename(columns={'fighter': 'opp_fighter'})
#     opp_df = opp_df.rename(columns={col: f'opp_{col}' for col in metric_cols})
#     merged = pd.merge(df, opp_df, on='fight_url', how='left')
#     # Keep only the row where the fighter is not equal to the opp_fighter.
#     merged = merged[merged['fighter'] != merged['opp_fighter']].reset_index(drop=True)
#     return merged

# def compute_opponent_stats_fights(df: pd.DataFrame, metric_cols: list, window_list: list = [3, 5]) -> pd.DataFrame:
#     """
#     Given a fight-level DataFrame with fighter metrics, add the opponent's metrics and compute
#     cumulative and rolling aggregates for them.
    
#     The new opponent columns will have the prefix 'opp_' and the aggregates will be named similarly,
#     e.g., 'cum_opp_total_str_landed_current', 'rolling_3_avg_opp_total_str_landed_in', etc.
#     """
#     # Add opponent stats columns.
#     df = add_opponent_stats(df, metric_cols)
#     # Compute cumulative stats for opponent metrics.
#     opp_metric_cols = [f"opp_{col}" for col in metric_cols]
#     opp_cum = compute_cumulative_stats(df, opp_metric_cols, fight_order_col='fight_order')
#     # Compute rolling aggregates for opponent metrics.
#     opp_roll = compute_rolling_aggregates(df, opp_metric_cols, window_list)
#     # Concatenate the new opponent columns.
#     return pd.concat([df, opp_cum, opp_roll], axis=1)

# def compute_stats_fights(raw_df: pd.DataFrame, window_list: list = [3, 5]) -> pd.DataFrame:
#     """
#     Given raw round-level data (with one row per fighter per round) that has an 'event_date' column,
#     this function first aggregates the data to fight-level (one row per fighter per fight) and then computes:
#       - Cumulative stats: _current (including the fight) and _in (incoming, before the fight)
#       - Fixed-window rolling aggregates (incoming) for the specified window sizes.
      
#     Parameters:
#       raw_df: The raw rounds DataFrame (must include at least 'fight_url', 'fighter', and 'event_date').
#       window_list: List of window sizes to compute rolling aggregates.
      
#     Returns:
#       DataFrame with aggregated fight-level data and additional cumulative/rolling columns.
#     """
#     # First, aggregate round-level data.
#     fight_df = aggregate_rounds_by_fight(raw_df)
    
#     # Ensure event_date is present and convert it.
#     if 'event_date' not in fight_df.columns:
#         raise ValueError("Missing 'event_date' column in aggregated data. Ensure it is merged from events.")
#     fight_df['event_date'] = pd.to_datetime(fight_df['event_date'], errors='coerce')
    
#     # Sort by fighter and event_date; assign fight order.
...
#     # Identify numeric metric columns to process.
#     keys = ['fight_url', 'fighter', 'event_date', 'fight_order']
#     numeric_cols = fight_df.select_dtypes(include=[np.number]).columns.tolist()
#     # Exclude our temporary keys and any columns that already begin with "cum_", "avg_", or "rolling_".
...
#     # Compute fixed-window rolling aggregates.
#     roll_df = compute_rolling_aggregates(fight_df, metric_cols, window_list)
    
#     # Combine the new columns.
#     result_df = pd.concat([fight_df, cum_df, roll_df], axis=1)
#     result_df.drop(columns=['fight_order'], inplace=True)
#     return result_df
