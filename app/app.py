...
def calculate_average_time_between_fights(fight_dates):
    """Calculate average interval between fights given a list of dates."""
...
def compute_additional_stats(fighter, fights_df):
    """Compute additional statistics for the fighter."""
...
def build_fighter_info_card(fighter, stats):
    """Return a Card with fighter's info grouped into categories."""
    ...
    return dbc.Card(
        [
            ...
            dbc.CardBody([
                ...
                html.P(f"Average Time Between Fights: {stats['avg_interval_days']} days"),
                html.Hr(),
...
def build_stats_card(fighter):
    """Return a Card with cumulative statistics and a graph."""
    return dbc.Card(
        [
            dbc.CardHeader(html.H4("Fight Results Statistics", className="text-center")),
            dbc.CardBody([
                dcc.Graph(
                    id="stats-graph",
...
def build_fight_history_table(fights_df):
    """Return a Card containing a table with fight history."""
...
def update_profile(selected_fighter_url):
    ...
    # Retrieve fighter record and all fights for this fighter
