...
def test_dummy_only():
    ...
    result_df = add_all_cumsum_columns(df.copy(), dummy_cols=["result"], numerical_cols=[], group_col="fighter_url")
    
    # After cleaning, the unique dummy values should be "win" and "loss"
    # For group A: [("win", row0), ("loss", row1), ("win", row2)]
    #   Cumulative: total_win = [1, 1, 2], total_loss = [0, 1, 1]
    # For group B: [("loss", row3), ("win", row4)]
    #   Cumulative: total_win = [0, 1], total_loss = [1, 1]
...
def test_numerical_only():
    # Create a DataFrame with a numerical column (note: non-numeric values included).
    df = pd.DataFrame({
        "fighter_url": ["A", "A", "B", "B"],
        "score": [1, 2, "3", "bad"]
    })
    # Apply the function with only numerical columns.
    result_df = add_all_cumsum_columns(df.copy(), dummy_cols=[], numerical_cols=["score"], group_col="fighter_url")
    
    # 'bad' should be coerced to NaN and then replaced with 0.
    # Group A: cumulative score: [1, 1+2=3]
    # Group B: cumulative score: [3, 3+0=3]
...
def test_both_dummy_numerical():
    ...
    result_df = add_all_cumsum_columns(df.copy(), dummy_cols=["result"], numerical_cols=["score"], group_col="fighter_url")
    
    # For group A:
    #   Row 0: "win" --> dummy: win=1, loss=0, score=1  => cumulative: total_win=1, total_loss=0, total_score=1
    #   Row 1: "loss" --> dummy: win=0, loss=1, score=None becomes 0  => cumulative: total_win=1, total_loss=1, total_score=1
    # For group B:
...
def test_custom_prefix_and_row_count():
    ...
    # For group X:
    #   Row 0: "yes" -> dummy: yes=1, no=0; value=5  => cumulative: cumsum_yes=1, cumsum_no=0, cumsum_value=5, count=1
    #   Row 1: "no"  -> dummy: yes=0, no=1; value=10 => cumulative: cumsum_yes=1, cumsum_no=1, cumsum_value=15, count=2
    # For group Y:
...
def test_non_string_dummy_column():
    ...
    result_df = add_all_cumsum_columns(df.copy(), dummy_cols=["category"], numerical_cols=[], group_col="group")
    
    # For group G1:
    #   Row 0: category 1  => dummy: 1=1, 2=0; cumulative: total_1=1, total_2=0; row_count=1
    #   Row 1: category 2  => dummy: 1=0, 2=1; cumulative: total_1=1, total_2=1; row_count=2
    # For group G2:
...
def test_empty_dataframe():
    ...
    result_df = add_all_cumsum_columns(df.copy(), dummy_cols=["dummy"], numerical_cols=["num"], group_col="group")
